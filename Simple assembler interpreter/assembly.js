...
function simple_assembler(program) {
    ...
    while(index < program.length){
      let target = program[index].split(" ")
      if(target[2] !== undefined && !target[2].match(/[A-Za-z]/)){
        target[2] = parseInt(target[2])
      }
      if(target[0] === 'mov'){
        if(typeof target[1] === 'string' && typeof target[2] === 'number'){
          register[target[1]] = target[2]
        }
        if(typeof target[1] === 'string' && typeof target[2] !== 'number'){
          register[target[1]] = register[target[2]]
        }
        index++
      }else if(target[0] === 'inc'){
        register[target[1]] += 1
        index++
      }else if(target[0] === 'dec'){
        register[target[1]] -= 1
        index++
      }else if(target[0] === 'jnz'){
        if(register[target[1]] === 0){
          index++
        }else if(register[target[1]] !== 0){
          index += target[2]
        }else if(target[1] !== 0){
          index += target[2]
        }
